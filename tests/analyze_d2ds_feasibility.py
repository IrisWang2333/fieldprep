...
# ============================================================================
# ANALYSIS 3: D2DS Segments - Fixed Before Saturday
# ============================================================================
print("\n" + "="*70)
print("ANALYSIS 3: D2DS Segments Fixed Before Saturday")
print("="*70)
print("\nQuestion: Among D2DS segments, how many are fixed before Saturday (survey start)?")
...
# Check if fixed before the week's Saturday
d2ds_potholes['week_saturday'] = d2ds_potholes['week_start']  # Week starts on Saturday
d2ds_potholes['fixed_before_saturday'] = (
    (d2ds_potholes['date_closed'].notna()) &
    (d2ds_potholes['date_closed'] < d2ds_potholes['week_saturday'])
).astype(int)

# Check fix timing within the week
...
print(f"  Fixed before Saturday (survey start): {d2ds_potholes['fixed_before_saturday'].sum():,} "
      f"({d2ds_potholes['fixed_before_saturday'].mean():.1%})")
print(f"  Mean days to fix: {d2ds_potholes['days_to_fix'].mean():.1f}")
...
fix_timing_by_week = d2ds_potholes.groupby('week_of_year').agg({
    'fixed_before_saturday': 'mean',
    'days_to_fix': 'mean'
}).reset_index()

# Save
fix_timing_file = analysis_output_dir / "d2ds_fix_timing.csv"
d2ds_potholes[['segment_id', 'week_start', 'date_requested', 'date_closed',
               'days_to_fix', 'fixed_before_saturday']].to_csv(fix_timing_file, index=False)
...
3. D2DS SEGMENTS FIX TIMING (Historical)
   - Fixed before Saturday: {d2ds_potholes['fixed_before_saturday'].sum():,} ({d2ds_potholes['fixed_before_saturday'].mean():.1%})
   - Mean days to fix: {d2ds_potholes['days_to_fix'].mean():.1f} days

   CONCLUSION: {'LOW CONTAMINATION' if d2ds_potholes['fixed_before_saturday'].mean() < 0.3 else 'HIGH CONTAMINATION'}
   {'✓ Most fixes occur after survey week starts' if d2ds_potholes['fixed_before_saturday'].mean() < 0.3 else '⚠ Many fixes occur before survey (potential contamination)'}
