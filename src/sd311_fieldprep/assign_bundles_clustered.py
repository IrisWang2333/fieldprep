...
def assign_bundles_for_date_clustered(
    ...
    bundles_per_interviewer: int = 4,
    max_interviewers: int | None = None,
    ...
    Args:
        ...
        max_interviewers: Maximum number of interviewers to use (None = use all)
    ...
    print(f"[Clustered Assignment] Loaded {len(interviewers)} interviewers for {date}")

    # Limit to max_interviewers if specified
    if max_interviewers is not None and len(interviewers) > max_interviewers:
        interviewers = interviewers[:max_interviewers]
